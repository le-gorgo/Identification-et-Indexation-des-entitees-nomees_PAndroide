...
def isParticule(mot):
    particules=['de','du',"d'","de"+u'\u2027'+"la", "de"+u'\u2027'+"La", "de"+u'\u2027'+"l'", "de"+u'\u2027'+"L'"]
...
text=text.replace(u'\u2026',' ')
text=text.replace(u'\u2019',"'")
text=text.replace("de la","de"+u'\u2027'+"la")
text=text.replace("de La","de"+u'\u2027'+"La")
text=text.replace("de l'","de"+u'\u2027'+"l'")
text=text.replace("de L'","de"+u'\u2027'+"L'")
text=text.replace("'","' ")
text=text.replace("  "," ")
            ...
            if isInitiales(mot) :
                ...
                if startsUpper(mot) :
                    ...
                    if not(exclus(mot2,exc)):
                        print ">>",mot2
                        f2.write("\t\t>>"+mot2+"\t["+motprec+']\n')
                        addDico(np,mot2)
                        if (startsUpper(motprec) and not (isPrefixe(motprec) or motprec[-1] in {',',')'} or exclus(motprec,exc)) ):
                            mot3=motprec+" "+mot2
                            print ">>>",mot3
                            f2.write("\t\t   >>>"+mot3+'\n')
                            addDico(np,mot3)
...
print "\nLIEUX"
for (mot,f) in sorted(dlieux.items(),lambda a,b:cmp(a[1],b[1])):
    print mot, ":", f
print "\nAUTHORS"
for (mot,f) in sorted(dauthors.items(),lambda a,b:cmp(a[1],b[1])):
    print mot, ":", f
print "\nCRITIQUES"
for (mot,f) in sorted(dcrit.items(),lambda a,b:cmp(a[1],b[1])):
    print mot, ":", f
print "\nOEUVRES"
for (mot,f) in sorted(doeuvres.items(),lambda a,b:cmp(a[1],b[1])):
    print mot, ":", f
print "\nPERSONNAGES"
for (mot,f) in sorted(dpers.items(),lambda a,b:cmp(a[1],b[1])):
    print mot, ":", f
print "\nINSTITUTIONS"
for (mot,f) in sorted(dinst.items(),lambda a,b:cmp(a[1],b[1])):
    print mot, ":", f
...
print "Nb References : ",len(np), "avant posfiltering : ", before, "lieux : ",len(dlieux), "auteurs : ",len(dauthors)
